...
Name2StationMcast::iterator getStation(char *n, char *s, ewConfig &cfg)
{
   Name2StationMcast::iterator sta_iter;
	sta_iter = cfg.sta_list.begin();
	while (sta_iter != cfg.sta_list.end()) {
#ifdef DEBUG_GS
		fprintf(stderr, "DEBUG: getStation() checking against station %s %s\n", (*sta_iter).second.net, (*sta_iter).second.site);
#endif
...
int GetConfig(char *configfile, ewConfig &cfg)
{
   ...
   char *sname_copy;
   cfg.maxstation=0;
   cfg.NoLatencyLogging=0;	// keeping this on by default
   cfg.gapdir[0]=0;
   cfg.latdir[0]=0;
   cfg.MSEED_RingKey = 0;
...
/* Read configuration parameters
   *****************************/
	 ...
	 else if ( k_its((char*) "NoLatencyLogging") ){
            cfg.NoLatencyLogging = 1;
	 }
	   ...
	   if( (str = k_str()) ){
	 ...
	 else if ( k_its( (char*) "MSEEDring" ) ) {
            if ( (str = k_str()) != NULL ) {
               if ( (cfg.MSEED_RingKey = GetKey(str)) == -1 )
               {
                  fprintf( stderr, "%s: Invalid MSEEDring <%s>. \n", cfg.Progname, str );
                  return -1;
               }
            }
	    ...
	    sta_iter = getStation(Net, Site, cfg);
...
/* After all files are closed, check flags for missed commands
   ***********************************************************/
   ...
   if ( GetType( (char*)"TYPE_HEARTBEAT", &(cfg.TypeHB) ) != 0 ) {
      fprintf( stderr,
              "%s: Invalid message type <TYPE_HEARTBEAT>\n",cfg.Progname);
      return( -1 );
   }
   if ( GetType( (char*)"TYPE_TRACEBUF2", &(cfg.TypeTrace) ) != 0 ) {
      fprintf( stderr,
              "%s: Invalid message type <TYPE_TRACEBUF2>; exiting!\n", 
	       cfg.Progname);
        return(-1);
   }
   if ( GetType( (char*)"TYPE_MSEED", &(cfg.TypeMSEED) ) != 0 ) {
      fprintf( stderr,
              "%s: Invalid message type <TYPE_MSEED>; exiting!\n", 
	       cfg.Progname);
        return(-1);
   }
   if ( GetType( (char*)"TYPE_ERROR", &(cfg.TypeErr) ) != 0 ) {
      fprintf( stderr,
              "%s: Invalid message type <TYPE_ERROR>\n", cfg.Progname);
      return( -1 );
   }

   /* build the datalogo's */
   setuplogo(&(cfg.DataLogo), cfg);
   cfg.DataLogo.type = cfg.TypeTrace;

   /* for optional mseed */
   setuplogo(&(cfg.MSEEDLogo), cfg);
   cfg.MSEEDLogo.type = cfg.TypeMSEED;

   if (cfg.gapdir[0] == 0 && cfg.latdir[0] == 0) {
      cfg.NoLatencyLogging = 1;
   }
