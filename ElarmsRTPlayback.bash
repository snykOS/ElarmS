...
if [ -e ${EW_INSTALL_HOME}/run/bin/wpr.conf ] && [ -e ${EW_INSTALL_HOME}/run/bin/EWP2r ] && [ -e ${EW_INSTALL_HOME}/run/bin/EWP2r.conf ] && [ -e ${EW_INSTALL_HOME}/run/bin/E2r ] && [ -e ${EW_INSTALL_HOME}/run/bin/E2r.conf ] && [ -e ${EW_INSTALL_HOME}/run/bin/dmr ] && [ -e ${EW_INSTALL_HOME}/run/bin/dmr.conf ]; then
  ...
  echo "echo \"ReplayRealTime                true\" >> wpr.conf"
...
# make sure ElarmS has a startstop_playback.d file
if [ ! -e ${EW_INSTALL_HOME}/run/params/startstop_playback.d ]; then
	echo "Creating startstop_playback.d file based on startstop.d"
	sed 's/#*\s*Process/#Process/;s/#*\s*Class/#Class/;s/My#Class/MyClass/' ${EW_INSTALL_HOME}/run/params/startstop_unix.d > ${EW_INSTALL_HOME}/run/params/startstop_playback.d
fi
# create a tankplayer command file
if [ ! -e tankplayer.d ]; then
	echo "creating a new tankplayer.d file"
	cp $EW_INSTALL_HOME/$EW_INSTALL_VERSION/params/tankplayer.d .
fi

if [ "$(grep "^WaveFile ${tnkfile}" tankplayer.d)"==NULL ]; then
	echo "Updating WaveFile in tankplayer.d file"
	sed -i 's@WaveFile.*@WaveFile '"${tnkfile}"'@' tankplayer.d
fi

echo "Switching to fast replay (using real time, 200 threads) on wpr.conf"
sed -i 's/.*ReplayRealTime.*/ReplayRealTime    true/' ${EW_INSTALL_HOME}/run/bin/wpr.conf
sed -i 's/.*ThreadCount.*/ThreadCount    200/' ${EW_INSTALL_HOME}/run/bin/wpr.conf
...
# Earthworm
if [[ ! `ps -ef | grep -v grep | grep $PWD | grep startstop startstop_playback.d 2> /dev/null` ]]; then
  echo Starting EarthWorm
  screen -dmS Earthworm bash -i -c ". ${EW_INSTALL_HOME}/run/params/ew_linux.bash;startstop startstop_playback.d"
else
...
if [[ ! `ps -ef | grep -v grep | grep $PWD | grep "EWP2r EWP2r.conf"` ]]; then
  echo Starting EWP2r
  screen -dmS ElarmSWP2 bash -i -c ". ${EW_INSTALL_HOME}/run/params/ew_linux.bash; cd ${EW_INSTALL_HOME}/run/bin;./EWP2r ChannelFile=channels.cfg params=EWP2r.conf | ts \"%FT%H:%M:%.S|\" | tee ${basedir}/EWP2r_${fname}.log"
else
  echo "EWP2r already running"
fi
echo ""
echo "Running tank player"
# tankplayer
tankplayer tankplayer.d
sleep 10
ElarmSStop.bash ElarmSWP2
while [[ `ps -ef | grep -v grep | grep $PWD | grep "E2r E2r.conf -replay"` ]]; do
	sleep 1
done
ElarmSStop.bash Earthworm
echo ""
IFS="
"
