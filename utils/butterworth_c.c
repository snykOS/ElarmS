 ...
 *  w = add_c(u,v)
 *  w = u + v
 *  @param u complex number to be added
 *  @param v complex number to be added
 ...
 *  w = mul_c(u,v)
 *  w = u * v
 *  @param u complex number to be multiplied
 *  @param v complex number to be multiplied
 ...
 *  a - real number
 *  u - complex number
 *  @param a real number to be multiplied
 *  @param v complex number to be multiplied
 *  @return complex number, result of multiplication
*/
 ...
 *  w = div_c(u,v)
 *  w = u/v
 *  @param u complex number to be divided
 *  @param v complex number to be divided
 *  @return complex number, result of division
*/
complex	div_c (complex u, complex v) {
    complex	w ;
 ...
 *  w = conjugate(u)
 *  @param u complex number to calculate complex conjugate 
 *  @return complex number, complex conjugate of input
*/
 ...
 *  denominator polynomial is z**2 + a1*z + a2
 *  numerator polynomial is z**2 + b1*z + b2
 *  fi = input array
 *  fo = output array
 *  npts = number of points
 *  @param a1 filter coefficient, denominator
 *  @param a2 filter coefficient, denominator
 *  @param b1 filter coefficient, numerator
 *  @param b2 filter coefficient, numerator
 *  @param npts number of points in array
 *  @param fi input array
 *  @param fo output array, modified by this function
 *  @param d1 value from earlier in filtered array (history)
 *  @param d2 value from earlier in filtered array (history)
*/
 ...
 *  @param FF output file name
 *  @param npts number of points in array
 *  @param ar double array of samples
*/
 ...
 *  Program calculates a continuous Butterworth low pass IIR with required cut off frequency.
 *  This program is limited to using an even number of poles, then a discrete filter is calculated 
 *  utilizing the bilinear transform.
 *  Methods used here follow those in Digital Filters and Signal Processing by Leland B. Jackson  
 *  @param fc = desired cutoff frequency
 *  @param dt = sample rate in seconds
 *  @param n = number of poles (MUST BE EVEN)
 *  @param p = pole locations (RETURNED)
 *  @param b = gain factor for filter (RETURNED)
 */
...
/**************************************************************************/
/** Routine to compute lowpass filter poles for Butterworth filter
 *  Program calculates a continuous Butterworth highpass IIR. First a low pass filter is 
 *  calculated with required cut off frequency. Then this filter is converted to a high 
 *  pass filter. This program is limited to using an even number of poles, then a discrete 
 *  filter is calculated utilizing the bilinear transform.
 *  Methods used here follow those in Digital Filters and Signal Processing by Leland B. Jackson  
 *  @param fc = desired cutoff frequency
 *  @param dt = sample rate in seconds
 *  @param n = number of poles (MUST BE EVEN)
...
/** Program to test filters for PASSCAL Instrument. These filters are Butterworth highpass and 
 *  lowpass filters both filters may be implemented or only one filter. This program limits the 
 *  number of poles to be either 0, 2 or 4. The program to calculate the pole position or to 
 *  filter does not have any limits.
 *  written by jcf   feb 1988
 *  modified  july 1993.
 *  @param data = input data array
 *  @param lowCutoff = low pass cutoff frequency
 *  @param highCutoff = high pass cutoff frequency
 *  @param numSamp = number of samples in array
 *  @param filterType = char indicating 'l' lowpass, 'h' highpass, any other character bandpass
 *  @param filterOrder = order of the filter can be 0, or an even number up to 12
 *  @param sampRate = sample rate                                                
 *  @return filtered array
 */
int* butter_filter_data(int *data, double lowCutoff, double highCutoff, int numSamp, 
                        char filterType, int filterOrder, int sampRate) {
    int *pOutData = NULL;
    int    i;
    complex	pl[12], ph[12] ;
    double	  b0l, b0h ;
...
/* Through with calculation of poles now calculate sweep for filtering     */
...
//		f1 = 1./(dt*2.) ;	/*  stop frequency = nyquist  */
    ...
    for (i=0; i < numSamp; i++ ) { af[i] = (double)data[i]; }
